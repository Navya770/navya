...
const products=[
 ...
 {name:"Water Bottle", price:199, img:"https://media.istockphoto.com/id/2183430912/photo/stainless-steel-thermos-bottles-with-lids.jpg?b=1&s=612x612&w=0&k=20&c=RugzEUj7oj85vTgyzI0RTUnhKixwPyyqDnzDxkPljno="},
 {name:"Wallet", price:399, img:"https://images.pexels.com/photos/910122/pexels-photo-910122.jpeg"}
];
let cart=JSON.parse(localStorage.getItem('cart')||"[]");
function saveCart(){localStorage.setItem('cart',JSON.stringify(cart));}
function showCount(){document.getElementById('count').innerText=cart.length;}
function registerUser(){
 localStorage.setItem('user',ruser.value);
 localStorage.setItem('pass',rpass.value);
 alert('Registered');
...
function loginUser(){
 if(luser.value===localStorage.getItem('user') && lpass.value===localStorage.getItem
('pass')){
 alert('Login Success');
 location.href="catalog.html";
...
function loadCart(){
 showCount();
 document.getElementById('cartItems').innerHTML=
 cart.map((c,i)=>`<div class='cart-item'>
 <span>${c.name}</span><span>₹${c.price}</span>
 <button onclick='removeItem(${i})'>X</button></div>`).join('');
}
